...
export function parseDoc(sourceFile: SourceFile, tc: TypeChecker): Doc {
  ...
  // parse doc comments
  const commentDoc = sourceFile.statements.map(parseDocComment).reduce((a, b) => ({ ...a, ...b }), {})
...
function parsePropDeclaration(name: string, node: Node, tc: TypeChecker): [PropKind, Prop[]] | undefined {
  ...
  if (v && v[0] === 'cesiumEventProps' && isObjectLiteralExpression(v[1])) {
    return v[1].properties
      .map<[string, string] | undefined>((node2) => {
        if (isPropertyAssignment(node2) && isIdentifier(node2.name) && isStringLiteral(node2.initializer)) {
